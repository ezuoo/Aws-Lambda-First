...
function MyPage(props) {
    ...
    return (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', 
            ...
            <div id={viewId} style={{ display: 'flex', flexDirection: 'column', visibility: 'hidden', position: 'absolute'  }} >
                <label>Email : {user === undefined ? 'null' : user.email}</label>
                <label>Name : {user === undefined ? 'null' : user.name}</label>
            </div>
        </div>
    )
}
